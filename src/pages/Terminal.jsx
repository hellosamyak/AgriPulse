...
export default function Terminal() {
  ...
  // 🌾 Fetch terminal analytics (with caching)
  const fetchData = async (
    ...
    try {
      // ✅ 1️⃣ Try cached first (instant)
      const cached = await api.get("/terminal/cached");
      if (cached.data) {
        setData(cached.data);
        setLoading(false);
        return;
      ...
      console.warn("Cached terminal not ready, fetching live...");
    }

    // ✅ 2️⃣ Fallback: fetch live data if cache not found
    try {
  ...
  // Auto-assign random destination port
  useEffect(() => {
    if (
      tradeParams.source &&
      Array.isArray(internationalOptions?.ports) &&
  ...
  const sellHigh = optimal_market?.sell_high ?? [];
  const buyLow = optimal_market?.buy_low ?? [];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-100">
              AgriPulse Terminal: Our Intelligent Decision Engine
            </h1>
